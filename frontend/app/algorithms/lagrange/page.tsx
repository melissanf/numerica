...
export default function LagrangePage() {
  ...
  async function handleSolve() {
    ...
    try {
      ...
      const res = await fetch(`/api/axe3/interpolation/lagrange`, {
        method: 'POST',
  ...
  return (
    <>
      <Header title="Lagrange" breadcrumb={['Interpolation Et Approximation', 'Lagrange']} />
            ...
            <div className="bg-card border border-border rounded-xl p-6 sticky top-24 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-input border border-border text-foreground rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                />
              </div>
              ...
              <button
                onClick={handleSolve}
                disabled={loading}
                className="w-full bg-blue-500/80 hover:bg-blue-600 text-white font-semibold py-2 rounded-lg transition-colors disabled:opacity-50"
              >
