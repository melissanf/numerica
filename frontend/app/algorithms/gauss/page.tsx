...
export default function GaussPage() {
  ...
  async function handleSolve() {
    ...
    setResult(null);
    try {
      const res = await fetch(`/api/axe2/gauss`, {
        method: 'POST',
  ...
  return (
    <>
      <Header title="Gauss" breadcrumb={['Résolution Des Systèmes Linéaires', 'Gauss']} />
          ...
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-xl p-6 sticky top-24 space-y-5">
                    ...
                    <div key={i} className="flex gap-1">
                      {row.map((val, j) => (
                        <input
                          ...
                          className="w-full bg-input border border-border text-foreground rounded px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                        />
                      ))}
                    </div>
                  ))}
              ...
              <button
                onClick={handleSolve}
                disabled={loading}
                className="w-full bg-indigo-500/80 hover:bg-indigo-600 text-white font-semibold py-2 rounded-lg transition-colors disabled:opacity-50"
              >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {result.solution.map((val: number, i: number) => (
                      <div key={i} className="bg-input rounded-lg p-4">
                        ...
                        <p className="text-lg font-bold text-indigo-400">{val.toFixed(8)}</p>
                      </div>
                    ))}
                  </div>
                </div>
