...
export default function GaussSeidelPage() {
  ...
  function resizeMatrix(newN: number) {
    ...
    setA(
      Array.from({ length: newN }, (_, i) =>
        Array.from({ length: newN }, (_, j) => A[i]?.[j] ?? 0),
      ),
    );
  ...
  const convergenceData =
    result?.erreurs?.map((e: number, i: number) => ({
      iteration: i + 1,
      error: e,
    })) ?? [];
  ...
  return (
    <>
      <Header
        title="Jacobi"
        breadcrumb={["Résolution Des Systèmes Linéaires", "Jacobi"]}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-xl p-6 sticky top-24 space-y-5">
              ...
              {error && (
                <div className="p-3 rounded-lg border border-red-400/30 bg-red-500/10 text-red-400 text-xs">
                  {error}
                </div>
              )}
            </div>
            ...
            {!result && (
              <div className="bg-card border border-border rounded-xl p-12 flex items-center justify-center min-h-96">
                <p className="text-muted-foreground text-center">
                  Fill in the matrix and click Solve
                </p>
              </div>
            )}
          </div>
