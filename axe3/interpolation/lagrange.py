import numpy as np
from typing import Tuple, Callable


def lagrange_basis(i: int, x_nodes: np.ndarray, x: float) -> float:
    result = 1.0
    for j in range(len(x_nodes)):
        if i != j:
            result *= (x - x_nodes[j]) / (x_nodes[i] - x_nodes[j])
    return result


def lagrange_interpolate(x_nodes: np.ndarray, y_nodes: np.ndarray, x: float) -> float:
    result = 0.0
    for i in range(len(x_nodes)):
        result += y_nodes[i] * lagrange_basis(i, x_nodes, x)
    return result


def get_lagrange_polynomial_str(x_nodes: np.ndarray, y_nodes: np.ndarray) -> str:
    n = len(x_nodes)
    polynomial_terms = []

    for i in range(n):
        if y_nodes[i] == 0:
            continue

        # Build L_i(x) term
        numerator_factors = []
        denominator_factors = []

        for j in range(n):
            if i != j:
                numerator_factors.append(f"(x - {x_nodes[j]:.4g})")
                denominator_factors.append(f"({x_nodes[i]:.4g} - {x_nodes[j]:.4g})")

        numerator_str = " * ".join(numerator_factors)
        denominator_value = 1.0
        for j in range(n):
            if i != j:
                denominator_value *= x_nodes[i] - x_nodes[j]

        coeff = y_nodes[i] / denominator_value
        polynomial_terms.append(f"{coeff:.6g} * {numerator_str}")

    result = " + ".join(polynomial_terms)
    result = result.replace(" + -", " - ")

    return f"P(x) = {result}"


def lagrange_error_table(
    x_nodes: np.ndarray,
    y_nodes: np.ndarray,
    f: Callable[[float], float],
    test_points: np.ndarray,
) -> list:
    errors = []

    for x in test_points:
        p_x = lagrange_interpolate(x_nodes, y_nodes, x)
        f_x = f(x)
        error = abs(f_x - p_x)
        relative_error = error / abs(f_x) if f_x != 0 else 0

        errors.append(
            {
                "x": x,
                "f(x)": f_x,
                "P(x)": p_x,
                "error": error,
                "relative_error": relative_error,
            }
        )

    return errors


# Test functions for validation
def cos_function(x: float) -> float:
    """Test function: cos(x)"""
    return np.cos(x)


def exp_function(x: float) -> float:
    """Test function: exp(-1/(1+x²))"""
    return np.exp(-1 / (1 + x**2))


def runge_function(x: float) -> float:
    """Test function: 1/(1+25x²) - demonstrates Runge phenomenon"""
    return 1 / (1 + 25 * x**2)


def discrete_points() -> Tuple[np.ndarray, np.ndarray]:
    """Test points from TP4 Ex3: (0,0), (1,3), (5,2), (8,2)"""
    x_nodes = np.array([0, 1, 5, 8], dtype=float)
    y_nodes = np.array([0, 3, 2, 2], dtype=float)
    return x_nodes, y_nodes


if __name__ == "__main__":
    # Test 1: Discrete points
    print("=" * 60)
    print("TEST 1: Discrete Points")
    print("=" * 60)
    x_nodes, y_nodes = discrete_points()
    print(f"Nodes: x = {x_nodes}, y = {y_nodes}")

    # Evaluate at the nodes (should match exactly)
    print("\nEvaluation at nodes:")
    for i, x in enumerate(x_nodes):
        p_x = lagrange_interpolate(x_nodes, y_nodes, x)
        print(f"  P({x}) = {p_x:.6f} (should be {y_nodes[i]})")

    # Evaluate at some intermediate points
    print("\nEvaluation at intermediate points:")
    x_test = np.array([0.5, 2.0, 6.0])
    for x in x_test:
        p_x = lagrange_interpolate(x_nodes, y_nodes, x)
        print(f"  P({x}) = {p_x:.6f}")

    print(f"\n{get_lagrange_polynomial_str(x_nodes, y_nodes)}")

    # Test 2: Runge function with uniform nodes
    print("\n" + "=" * 60)
    print("TEST 2: Runge Function (1/(1+25x²)) - Runge Phenomenon")
    print("=" * 60)

    for n in [4, 8, 10]:
        x_nodes_uniform = np.linspace(-1, 1, n)
        y_nodes_runge = np.array([runge_function(x) for x in x_nodes_uniform])

        x_test = np.linspace(-1, 1, 20)
        errors = lagrange_error_table(
            x_nodes_uniform, y_nodes_runge, runge_function, x_test
        )

        max_error = max(e["error"] for e in errors)
        mean_error = np.mean([e["error"] for e in errors])

        print(f"\nn = {n} uniform nodes:")
        print(f"  Max error: {max_error:.8f}")
        print(f"  Mean error: {mean_error:.8f}")

    # Test 3: cos function
    print("\n" + "=" * 60)
    print("TEST 3: Cosine Function")
    print("=" * 60)

    x_nodes_cos = np.linspace(-4 * np.pi, 4 * np.pi, 8)
    y_nodes_cos = np.array([cos_function(x) for x in x_nodes_cos])

    x_test = np.linspace(-4 * np.pi, 4 * np.pi, 15)
    errors = lagrange_error_table(x_nodes_cos, y_nodes_cos, cos_function, x_test)

    print(f"Nodes: {len(x_nodes_cos)} points over [-4π, 4π]")
    print(f"Max error: {max(e['error'] for e in errors):.8f}")
    print(f"Mean error: {np.mean([e['error'] for e in errors]):.8f}")
